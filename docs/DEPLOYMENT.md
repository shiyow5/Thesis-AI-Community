...
[NSSM](https://nssm.cc/) で「OS 起動時に自動起動・クラッシュ時に自動再起動」するサービスにします。

### 2-1. NSSM の導入とサービス登録
```powershell
# nssm.exe を入手（https://nssm.cc/download）してパスを通すか、フルパスで実行
...
### 2-2. 運用コマンド
```powershell
nssm status  ThesisAICommunity   # 状態確認
nssm restart ThesisAICommunity   # 再起動
nssm stop    ThesisAICommunity   # 停止
nssm remove  ThesisAICommunity confirm   # サービス削除
```

> **代替**: NSSM を使わず Windows タスクスケジューラで「スタートアップ時／失敗時に再実行」を構成しても可。

---

## 3. 可用性・再起動耐性

- **自動再起動**: NSSM がクラッシュ時に自動復帰（OS 起動時も自動起動）
...
### 注意
- Windows Update の自動再起動でプロセスが落ちることがある → NSSM 自動起動でカバー
...
ログは `logs\bot.log`（標準出力）/ `logs\bot.err.log`（エラー）に出力されます（NSSM 設定時）。
